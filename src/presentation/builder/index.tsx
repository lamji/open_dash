...
function getSlotPlacementStyle(block: LayoutBlock, slotIdx: number): CSSProperties {
  ...
  if (slotIdx === block.slots.length - 1) {
    return { gridColumn: `${13 - span} / span ${span}` };
  }

  const start = spans.slice(0, slotIdx).reduce((sum, current) => sum + current, 1);
...
export default function BuilderShell() {
  ...
  const renderSlotContent = (block: LayoutBlock, slot: LayoutSlot, slotIdx: number, depth: number) => {
    ...
    return (
      <div
        key={slotIdx}
        className={`relative w-full min-h-[300px] rounded-xl ${!hasCustomStyle && !widget && !hasChildren ? "bg-[#ECECEC]" : ""} ${hasChildren ? "border border-slate-300 bg-slate-100/80 p-3" : widget ? (!hasCustomStyle ? "bg-white shadow-sm border border-slate-200 p-5" : "") : ""}`}
        style={{ ...slotStyle, ...slotPlacementStyle }}
        data-test-id={`builder-slot-${block.id}-${slotIdx}`}
      >
        {!previewMode && !widget && !hasChildren && (
        ...
        {!widget && !hasChildren && !previewMode && (
          <div className="flex flex-col items-center justify-center h-full min-h-[200px] gap-3">
            ...
            <div className="flex w-full max-w-[280px] items-center justify-center">
              <Button
                size="icon"
                className="h-9 w-9 rounded-full border border-[var(--primary)]/20 bg-[var(--primary)] text-[var(--primary-foreground)] shadow-sm transition-colors hover:bg-[var(--primary)]/90"
        ...
        {hasChildren && (
          <div className={`${widget ? "mt-4" : "mt-0"} space-y-3`} data-test-id={`builder-slot-children-${block.id}-${slotIdx}`}>
            {!previewMode && (
    ...
    return (
      ...
      >
        ...
        <div className={`${getBlockContainerClass(block)} mt-0`} style={getBlockContainerStyle(block)} data-test-id={`builder-block-grid-${block.id}`}>
          {block.slots.map((slot, slotIdx) => renderSlotContent(block, slot, slotIdx, depth))}
        </div>
      </div>
    );
  };
