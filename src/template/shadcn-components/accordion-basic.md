# Shadcn UI: Basic Accordion Variant

## 🎯 Purpose
Provide a standardized, pre-made implementation pattern for the Shadcn UI `Accordion` component. This variant demonstrates a single-item collapsible accordion for FAQs or information disclosure.
...
## 🛠️ Implementation Example (`accordion-01.tsx`)
...
## 📋 Rules
1. Uses `Accordion`, `AccordionItem`, `AccordionTrigger`, and `AccordionContent` Shadcn components.
2. Supports `type='single'` with `collapsible` behavior for better UX.
3. Provides a `defaultValue` to show the first item by default.
4. Uses `text-muted-foreground` for content to provide visual hierarchy.
