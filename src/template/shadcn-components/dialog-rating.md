# Shadcn UI: Dialog Rating Variant

## 🎯 Purpose
Provide a standardized, pre-made implementation pattern for the Shadcn UI `Dialog` component that acts as a feedback rating modal. This demonstrates integrating a custom styled `RadioGroup` for emoji ratings with `Textarea` and `Checkbox` components.

## 🛠️ Implementation Example (`dialog-rating.tsx`)
...
## 📋 Rules
1. Uses `Button`, `Checkbox`, `Dialog`, `Label`, `RadioGroup`, and `Textarea` Shadcn components.
2. Demonstrates using Tailwind CSS `has-*` pseudo-classes (e.g., `has-data-[state=checked]:bg-sky-600/10`) to restyle `<label>` wrappers based on hidden `<RadioGroupItem>` states.
3. Groups semantically related form fields within a `<fieldset>` with a `<legend>`.
