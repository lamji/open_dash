# Shadcn UI: Dialog Sticky Header Variant

## 🎯 Purpose
Provide a standardized, pre-made implementation pattern for the Shadcn UI `Dialog` component featuring a sticky header and a scrollable content area. This is ideal for long terms of service, product information, or forms that overflow the viewport, where the title must remain visible at all times.

## 🛠️ Implementation Example (`dialog-sticky-header.tsx`)
...
## 📋 Rules
1. Use `Dialog` (@/components/ui/dialog) paired with `ScrollArea` (@/components/ui/scroll-area).
2. The `DialogContent` requires specific height-limiting Tailwind classes to force the `ScrollArea` to handle overflow (e.g., `className='flex max-h-[min(600px,80vh)] flex-col gap-0 p-0 sm:max-w-md'`).
3. Place `DialogTitle` *outside* and *above* the `ScrollArea` component, applying a bottom border (`border-b`) to separate it visually.
4. Place the scrollable content inside the `ScrollArea` component.
5. In this pattern, the `DialogFooter` is placed *inside* the `ScrollArea` at the bottom, so it scrolls into view rather than remaining sticky.
