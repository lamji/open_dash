# Shadcn UI: Dialog OTP Verification Variant

## 🎯 Purpose
Provide a standardized, pre-made implementation pattern for the Shadcn UI `Dialog` component that acts as an OTP (One-Time Password) verification modal. This demonstrates integrating the external `input-otp` library with Shadcn components for a complete verification flow.

## 🛠️ Implementation Example (`dialog-otp-verification.tsx`)
...
## 📋 Rules
1. **Dependencies:** Requires `npm install input-otp lucide-react`
2. Demonstrates using `OTPInput` from `input-otp` directly within a Shadcn `Dialog`. Note that Shadcn also offers its own `<InputOTP>` component wrapper, but this pattern demonstrates custom slot rendering directly using the base library.
3. Uses state (`hasGuessed`) to toggle the UI dynamically between the verification input and the success state without closing the dialog.
4. Relies on the standard `cn` helper for dynamic class switching (e.g., turning the icon background green on success).
