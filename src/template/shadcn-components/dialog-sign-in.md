# Shadcn UI: Dialog Sign In Variant

## 🎯 Purpose
Provide a standardized, pre-made implementation pattern for the Shadcn UI `Dialog` component that acts as a Sign In modal. This demonstrates complex form layouts within a dialog, including gradients, icons, input fields, and alternative sign-in buttons (social logins).

## 🛠️ Implementation Example (`dialog-sign-in.tsx`)
...
## 📋 Rules
1. **HARD RULE**: You MUST combine `Dialog`, `Input`, `Label`, and `Button` components exactly as shown.
2. **HARD RULE**: You MUST use Tailwind CSS gradients on the `DialogContent` for the premium aesthetic.
3. **MANDATORY**: Incorporate a centered icon and title within the `DialogHeader`.
4. **MANDATORY**: Use a `flex-wrap` layout in `DialogFooter` to nicely align generic social login buttons.
