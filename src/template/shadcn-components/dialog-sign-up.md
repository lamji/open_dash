# Shadcn UI: Dialog Sign Up Variant

## 🎯 Purpose
Provide a standardized, pre-made implementation pattern for the Shadcn UI `Dialog` component that acts as a Sign Up modal. This demonstrates complex form layouts within a dialog, including gradients, multiple inputs, checkboxes, and alternate sign-in buttons (like Google).

## 🛠️ Implementation Example (`dialog-sign-up.tsx`)
...
## 📋 Rules
1. Uses `Dialog`, `Input`, `Label`, `Checkbox`, and `Button` components.
2. Demonstrates using Tailwind CSS gradients for the `DialogContent` background.
3. Shows how to structure a complex nested form within a dialog (though logically, the outer `<form>` wrapping `<DialogTrigger>` and `<DialogContent>` in this demo should often be avoided in actual React applications to prevent nested forms depending on the underlying structure).
4. Demonstrates adding a separator and alternative sign-in options within the `DialogFooter`.
