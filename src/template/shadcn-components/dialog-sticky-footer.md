# Shadcn UI: Dialog Sticky Footer Variant

## 🎯 Purpose
Provide a standardized, pre-made implementation pattern for the Shadcn UI `Dialog` component featuring a sticky footer and a scrollable content area. This is ideal for long terms of service, product information, or forms that overflow the viewport.

## 🛠️ Implementation Example (`dialog-sticky-footer.tsx`)
...
3. Place the scrollable content inside `ScrollArea` components nested within the `DialogHeader`.
4. The `DialogFooter` should be styled with an upper border (`border-t`) to separate it visually from the scrolling content.
