# Shadcn UI: Alert Dialog with Icon Variant

## 🎯 Purpose
Provide a standardized, pre-made implementation pattern for the Shadcn UI `AlertDialog` component that includes an icon in the header and a styled list of details.

## 🛠️ Implementation Example (`dialog-02.tsx`)
...
## 📋 Rules
1. Always import `AlertDialog` parts from `@/components/ui/alert-dialog`.
2. Use `lucide-react` for icons (e.g., `UploadIcon`).
3. Maintain the structured header format containing the icon wrapped in a styled `div`, followed by the `AlertDialogTitle` and `AlertDialogDescription`.
