# Shadcn UI: Dialog Fullscreen Variant

## 🎯 Purpose
Provide a standardized, pre-made implementation pattern for the Shadcn UI `Dialog` component that expands to fill the entire viewport, featuring a scrollable content area. This is ideal for immersive content or complex forms on smaller screens.

## 🛠️ Implementation Example (`dialog-full-screen.tsx`)
...
2. The `DialogContent` requires specific height and width classes using viewport units (e.g., `className='mb-8 flex h-[calc(100vh-2rem)] min-w-[calc(100vw-2rem)] flex-col justify-between gap-0 p-0'`) to simulate fullscreen mode while maintaining a small margin.
3. Place `ScrollArea` immediately within `DialogContent` to ensure the internal content scrolls properly while keeping the footer pinned at the bottom.
